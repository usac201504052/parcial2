...
class cliente(object):
    ...
    def grabar(self, topic, duracion):
        os.system('arecord -d {} -f U8 -r 8000 201504229_sent.wav'.format(duracion))
        with open('201504229_sent.wav', 'rb') as audio:
    ...
    def reproducirAudio(self, contenido = []):
        print("Reproduciendo")
        with open("201504229_receive.wav", "wb") as rAudio:   # JAPO Se abre el archivo de audio donde se desea guardar audio
            rAudio.write(contenido)                     # JAPO Se escribe el audio recibido sobre el archivo
            rAudio.close
        os.system('aplay 201504229_receive.wav') # JAPO Reproduce el archivo de audio
